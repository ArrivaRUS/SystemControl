...
struct BatteryInfo: Equatable {
    ...
    /// macOS придерживает зарядку заметно ниже 100%: воткнут адаптер, батарея
    /// не полная, но ток не идёт. Поведенческий признак — не зависит от
    /// недокументированных ключей вроде NotChargingReason и потому не ломается
    /// на новых macOS.
    ///
    /// Порог 90%, а не 100%: у почти полной батареи macOS штатно не доливает
    /// заряд (бережёт ресурс), и без порога мы бы объявляли «удержанием»
    /// совершенно нормальное плато на 93–99%.
    var chargeHeld: Bool {
        externalConnected && !isCharging && !fullyCharged && percent <= 90
    }
}
